...
import { DEFAULTS, DEFAULT_SUPPLIES, DEFAULT_DM_BOARD, readSupplies, imageIds, inlineReferences, KINDS, visible, editable, deletable, type Entry, type Member, type Settings } from './campaign';
...
function member(u:any):Member{return{id:u.id,name:u.name,role:u.role,active:u.active,changed:u.changed};}
...
async function session(req:Request,db:D1Database){const token=req.headers.get('cookie')?.split(';').map(x=>x.trim()).find(x=>x.startsWith(COOKIE+'='))?.slice(COOKIE.length+1);if(!token)return null;return await db.prepare('SELECT users.* FROM users JOIN sessions ON sessions.user_id = users.id WHERE sessions.token = ? AND sessions.expires > ? AND users.active = 1').bind(await digest(token),Date.now()).first<any>();}
function setCookie(token:string,req:Request,maxAge=604800){return `${COOKIE}=${token}; Path=/; HttpOnly; SameSite=Strict; Max-Age=${maxAge}${new URL(req.url).protocol==='https:'?'; Secure':''}`;}
...
async function allUsers(db:D1Database){return (await db.prepare('SELECT id,name,role,active,changed FROM users ORDER BY role,name').all()).results as Member[];}
...
function cleanData(kind:string,d:any){d=d&&typeof d==='object'?d:{};const result:any={images:Array.isArray(d.images)?d.images.slice(0,12).map((x:any)=>({id:cleanText(x?.id,80),caption:cleanText(x?.caption,240)})).filter((x:any)=>x.id):[]};
 if(kind==='pin'||kind==='event'){Object.assign(result,point(d));result.category=cleanText(d.category,40,'luogo');if(kind==='pin'){result.markerImage=cleanText(d.markerImage,80);result.markerIcon=cleanText(d.markerIcon,30,'pin');}}
 ...
 if(kind==='journal'){result.session=Math.floor(number(d.session,1,10000,1));result.date=cleanText(d.date,40);}
 if(kind==='treasure'){result.quantity=number(d.quantity,0,1000000,1);result.value=number(d.value,0,1000000000,0);result.holder=cleanText(d.holder,100,'Gruppo');result.category=cleanText(d.category,60,'Oggetto');}
...
export async function handleCampaign(req:Request,env:Bindings):Promise<Response>{
 ...
 const bucket=await digest((req.headers.get('cf-connecting-ip')||'unknown')+':'+name);const now=Date.now();
 const attempt=await db.prepare('INSERT INTO attempts (key,count,reset) VALUES (?,1,?) ON CONFLICT(key) DO UPDATE SET count = CASE WHEN reset < ? THEN 1 ELSE count + 1 END, reset = CASE WHEN reset < ? THEN ? ELSE reset END RETURNING count').bind(bucket,now+900000,now,now,now+900000).first<any>();
 if(attempt.count>10)fail(429,'Troppi tentativi. Riprova fra 15 minuti.');
 ...
 const token=random();await db.batch([db.prepare('INSERT INTO sessions (token,user_id,expires) VALUES (?,?,?)').bind(await digest(token),u.id,now+604800000),db.prepare('DELETE FROM attempts WHERE key = ? OR reset < ?').bind(bucket,now),db.prepare('DELETE FROM sessions WHERE expires < ?').bind(now)]);
 ...
 const state={user,users:users.filter(u=>u.active||user.role==='dm').map(u=>({...u,changed:u.id===user.id?u.changed:undefined})),records:allowed.map(r=>({...r,body:r.body.replace(/\[\[([^\]|]+)(?:\|([^\]]*))?\]\]/g,(token,id)=>ids.has(id)?token:'[['+id+']]'),links:r.links.filter(id=>ids.has(id)),folder:ids.has(r.folder)?r.folder:''})),settings:{...DEFAULTS,locationRadiusMiles:.25,...JSON.parse(s.value)},settingsVersion:s.version,supplies:readSupplies(suppliesRow?JSON.parse(suppliesRow.value):{}),suppliesVersion:suppliesRow?.version||0,...(user.role==='dm'?{dmBoard:dmRow?JSON.parse(dmRow.value):DEFAULT_DM_BOARD,dmBoardVersion:dmRow?.version||0}:{})};
 if(path==='/api/export'){
 ...
 const r=await db.prepare('DELETE FROM records WHERE id = ? AND version = ?').bind(old.id,b.version).run();if(!r.meta.changes)fail(409,'Il contenuto è stato modificato. Ricaricalo prima di eliminarlo.');return json({ok:true});
 }
 ...
 if(['secret','faction'].includes(kind)&&user.role!=='dm')fail(403,'Questa sezione è riservata al DM.');
 ...
 if(kind==='secret')audience=users.filter(u=>u.role==='dm').map(u=>u.id);
 ...
 if(old){const result=await db.prepare('UPDATE records SET title = ?, body = ?, audience = ?, folder = ?, data = ?, links = ?, updated = ?, editor = ?, version = version + 1 WHERE id = ? AND version = ?').bind(...values,id,b.version).run();if(!result.meta.changes)fail(409,'Qualcuno ha aggiornato questa voce. Il tuo testo resta qui: copialo, chiudi e riapri per unire le modifiche.');}
 ...
 const file=await env.BUCKET.get(id);if(!file)fail(404,'Immagine non disponibile.');return new Response(file.body,{headers:{'Content-Type':u.mime,'Cache-Control':'private, no-store','X-Content-Type-Options':'nosniff'}});
 }
