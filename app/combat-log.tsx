...
export default function CombatLog({events,compact=false}:{events:CombatEvent[];compact?:boolean}){
 ...
 return <section className={'battle-reader '+(compact?'battle-reader--compact':'')} aria-label="Registro delle azioni">
  <header className="battle-reader-heading"><span className="battle-reader-seal"><Swords size={22} aria-hidden="true"/></span><div><p className="battle-eyebrow">{compact?'Il combattimento, azione per azione':'La memoria dello scontro'}</p><h3>{compact?'Ultime azioni':'Cronaca del combattimento'}</h3></div><span className="battle-record-count">{events.length}<small>registrazioni</small></span></header>
  {!events.length?<div className="battle-empty"><BookOpen size={28} aria-hidden="true"/><h4>La prima pagina è ancora da scrivere</h4><p>Attacchi, magie, condizioni e decisioni compariranno qui quando il DM li registra.</p></div>:<>
   {!compact&&<>
    <div className="battle-metrics" aria-label="Riepilogo dell’intero registro">
     <div><span><Swords size={14}/>Azioni registrate</span><strong>{number(summary.actions)}</strong><small>{summary.failed?summary.failed+' con esito mancato o fallito':'Ogni gesto annotato'}</small></div>
   ...
   {!compact&&<p className="battle-result-count" role="status">{filtering?count+(count===1?' registrazione corrispondente':' registrazioni corrispondenti'):count+(count===1?' registrazione da leggere':' registrazioni da leggere')}{count>shownCount?' · mostrate '+shownCount:''}</p>}
   {compact&&count>recentLimit&&<button type="button" className="battle-load-previous" onClick={()=>setRecentLimit(value=>value+RECENT_SIZE)}><ChevronUp size={16}/>Mostra {Math.min(RECENT_SIZE,count-recentLimit)} registrazioni precedenti<span>{count-recentLimit} ancora da leggere</span></button>}
   {!count?<div className="battle-empty battle-empty--filtered"><Search size={25} aria-hidden="true"/><h4>{filtering?'Nessuna azione corrisponde ai filtri':'In attesa della prima azione'}</h4><p>{filtering?'Prova un altro nome o una parola più breve.':'Sono presenti solo preparazione e cambi turno: puoi includerli nel registro con il controllo qui sopra.'}</p>{filtering&&<button type="button" className="battle-reset" onClick={reset}><RotateCcw size={14}/>Mostra tutte le azioni</button>}</div>:<div className="battle-chapters">{displayed.map(chapter=><RoundChapter key={chapter.id} chapter={chapter} compact={compact}/>)}</div>}
   {!compact&&count>limit&&<div className="battle-load-more"><span>Hai letto {shownCount} registrazioni su {count}</span><button type="button" onClick={()=>setLimit(value=>value+PAGE_SIZE)}>Continua la cronaca<ChevronDown size={16}/><small>Altre {Math.min(PAGE_SIZE,count-limit)}</small></button></div>}
